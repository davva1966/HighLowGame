...
public class ColorUtils {
	...
	public static Color[] generateVisuallyDistinctColors(int ncolors, float minComponent, float maxComponent) {
		return generateVisuallyDistinctColors(ncolors, minComponent, maxComponent, 1);
	}

	/*
	 * Returns an array of ncolors RGB triplets such that each is as unique from the rest as possible and each color has at least one component greater than minComponent and one less than maxComponent. Use min == 1 and max == 0 to include the full RGB color range.
	 * 
	 * Warning: O N^2 algorithm blows up fast for more than 100 colors.
	 */
	public static Color[] generateVisuallyDistinctColors(int ncolors, float minComponent, float maxComponent, int repeatPerColor) {
		...
		if (repeatPerColor > 1) {
			int count = 0;
			Color[] colors = new Color[size * repeatPerColor];
			for (Color color : rgbs) {
				for (int i = 0; i < repeatPerColor; i++) {
					colors[count] = color;
					count++;
				}
			}
	...
	private static float[] randYUVinRGBRange(float minComponent, float maxComponent) {
		while (true) {
			...
			if (0 <= r && r <= 1 && 0 <= g && g <= 1 && 0 <= b && b <= 1 && (r > minComponent || g > minComponent || b > minComponent) && // don't want all dark components
